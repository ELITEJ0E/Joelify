...
import { Clock, Music, TrendingUp, Calendar, BarChart3 } from "lucide-react"
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Insights into your listening habits</p>
        </div>
        ...
        <Card className="p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold">{stats.topTracks.length}</p>
            </div>
          </div>
        </Card>
      </div>
          ...
          {stats.playsByDay.map((day) => (
            <div key={day.day} className="flex-1 flex flex-col items-center gap-2">
              ...
              <div className="text-center">
                ...
                <p className="text-sm font-medium">{day.plays}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-3">
            {stats.topTracks.map((track, index) => (
              <div key={track.id} className="flex items-center gap-3">
                ...
                <div className="text-sm font-medium text-muted-foreground">{track.plays} plays</div>
              </div>
            ))}
          </div>
        </Card>
          ...
          <div className="space-y-3">
            {stats.topArtists.map((artist, index) => (
              <div key={artist.name} className="flex items-center gap-3">
                ...
                <div className="text-sm font-medium text-muted-foreground">{artist.plays} plays</div>
              </div>
            ))}
          </div>
        </Card>
